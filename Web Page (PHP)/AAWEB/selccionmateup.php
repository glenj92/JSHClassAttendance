...
<div id="peP" style="border: 2px solid #33B5E5; border-radius:8px; margin-top:10px; margin-bottom:10px; padding-top:5px; padding-bottom:5px; background:#33B5E5;">
  ...
  <center>
  <?php
 echo" <a href='formupdategru.php?cod=".$cod."&cur=".$codc."&mate=".$codm."&ano=".$ano."&per=".$per."&jor=".$jor."&doc=".$idd."' class='button button-border' style='margin-top:5px; margin-bottom:5px;'>Atras</a>";
    ?>
    </center>
...
if($busqueda!=""&&$tipob!=""){
	if($tipob=="id"){
	$consulta=mysql_query("select * from asignatura_as where codMate='".$busqueda."' and codCurso='".$codc."'");
	}else if($tipob=="n"){
	$consulta=mysql_query("select * from asignatura_as where nomMate like '%".$busqueda."%' and codCurso='".$codc."'");
	}

}else{
	$consulta=mysql_query("select * from asignatura_as where codCurso='".$codc."'");
}
...
<table width='100%' border='1' cellspacing='2' cellpadding='2'>
  ...
  <tr>
	...
	<form id='form1' name='form1' method='post' action='selccionmateup.php?cod=".$cod."&cur=".$codc."&mate=".$codm."&ano=".$ano."&per=".$per."&jor=".$jor."&doc=".$idd."'>
<th>
  	<p>
...
while($fi=mysql_fetch_array($consulta)){
	...
	<td><a href='formupdategru.php?cod=".$cod."&cur=".$codc."&scodmup=".$fi[0]."&ano=".$ano."&per=".$per."&jor=".$jor."&doc=".$idd."'>Seleccionar</a></td>
